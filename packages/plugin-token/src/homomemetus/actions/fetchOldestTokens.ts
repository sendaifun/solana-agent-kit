...
import { z } from "zod";
import { fetch_oldest_tokens } from "../tools/fetch_oldest_tokens";

const fetchOldestTokensAction: Action = {
  name: "HOMOMEMETUS_FETCH_OLDEST_TOKEN",
  description: "Oldest token list from token list created in 24h",
  similes: ["fetch oldest token list", "get oldest token list"],
  examples: [
    [
      {
        input: {
        ...
        explanation: "Oldest token list successfully fetched",
      },
    ],
  ],
  schema: z.object({
  ...
  handler: async (agent: SolanaAgentKit, input: Record<string, any>) => {
    try {
      ...
      const result = await fetch_oldest_tokens(limit);
      return {
        status: "success",
        data: result,
        message: "Oldest token list successfully fetched",
      };
    } catch (error: any) {
      return {
        status: "error",
        message: `Failed to fetch oldest token list ${error.message}`,
        code: error.code || "HOMOMEMETUS_FETCH_OLDEST_TOKEN_FALIED",
      };
    }
  },
};

export default fetchOldestTokensAction;
