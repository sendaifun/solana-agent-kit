...
Use the same params (offerPublicKey, collateralAmount, leverage) for LAVARAGE_OPEN_POSITION to execute.`,
    ...
    [
      {
        ...
        output: {
          status: "success",
          quote: { outAmount: "15038", priceImpactPct: "0" },
        },
  ...
  schema: z.object({
    offerPublicKey: z
      .string()
      .describe("Offer public key from LAVARAGE_LIST_TOKENS"),
    collateralAmount: z
      .string()
      .describe("Collateral in quote token smallest units"),
  ...
  handler: async (agent, input) => {
    try {
      const quote = await lavarageGetQuote(
        agent,
