...
import { z } from "zod";
import { fetch_tokens_by_initializers } from "../tools/fetch_tokens_by_initializers";

const fetchTokensByInitializersAction: Action = {
  name: "HOMOMEMETUS_FETCH_TOKENS_BY_INITIALIZER",
  description: "Initializer-address-list-based token filter",
  similes: ["Fetch tokens by initializer", "Get tokens by initializer"],
  exmaples: [
    {
      input: {
        addresses: [
        ...
        data: [
          {
            ...
            bondingcurve_pubkey: "4HJYQykXPbkh43oZGoRtNNb39KMX7y4VUihV8qmbXMQy",
          ...
          },
          {
            ...
            initial_buy_amount: 11560235.054306,
            initial_buy_sol_amount: 0.326732673,
            bondingcurve_pubkey: "28bQtHwe8pUYWpkZ2RBJnyZvJP94fkMiFWtWUEqcqHZo",
            market_cap: 28.571317633414253,
            name: "Fart Walking",
            symbol: "FW",
          ...
          },
          {
            ...
            bondingcurve_pubkey: "DDKdE15qcwdr6kXkWi9tTjH1Zf6MsFCdAdLG87yf8cmo",
      ...
      explanation:
        "Filtered token list by initializer addresses successfully fetched",
    },
  ],
  schema: z.object({
  ...
  handler: async (agent: SolanaAgentKit, input: Record<string, any>) => {
    try {
      ...
      const result = await fetch_tokens_by_initializers(addresses, sort);

      return {
        status: "success",
        data: result,
        message: "Token list by initializer address list successfully fetched",
      };
    } catch (error: any) {
      return {
        status: "error",
        message: `Failed to fetch token list by initializer addresses list ${error.message}`,
        code: error.code || "HOMOMEMETUS_FETCH_TOKENS_BY_INITIALIZER_FALIED",
      };
    }
  },
};

export default fetchTokensByInitializersAction;
