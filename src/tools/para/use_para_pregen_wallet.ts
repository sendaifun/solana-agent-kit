...
class ParaSolanaWeb3SignerAdapter extends ParaSolanaWeb3Signer {
  ...
  constructor(para: ParaServer, connection: any) {
    ...
    this.publicKey = this.sender
      ? new PublicKey(this.sender)
      : new PublicKey("11111111111111111111111111111111");
  }
}

export async function useParaPregenWallet(
  ...
  try {
    if (!userShare) {
      throw new Error(
        "Provide `userShare` in the request body to create a pre-generated wallet.",
      );
    }
    ...
    const solanaSigner = new ParaSolanaWeb3SignerAdapter(
      para,
      agent.connection,
    );
